...
public class JwtUtil {
	 ...
	 private String secret = "bilgeadam";
	    ...
	    public Boolean validateToken(String token, UserDetails userDetails) {
	        ...
	        final String tc = extractTc(token);
	        // here we are validation the username and then check the token is expired or not
	        return (tc.equals(userDetails.getUsername()) && !isTokenExpired(token));
	    }
